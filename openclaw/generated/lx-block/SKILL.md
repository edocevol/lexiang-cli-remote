...
1. 查看当前状态 - 调用 `lx-block-table-get` 工具
   - block_id: `tbl_xxx`
   - format: `table`

2. 修改 - 调用 `lx-block-table-set` 工具
   - block_id: `tbl_xxx`
   - row: `2`
   - col: `1`
   - text: `修正值`

3. 验证结果 - 调用 `lx-block-table-get` 工具
   - block_id: `tbl_xxx`
   - format: `json`

### 替换文档中的某个章节

1. 查看文档树，定位目标章节 - 调用 `lx-block-tree` 工具
   - block_id: `root_xxx`

2. 一键替换 - 调用 `lx-block-replace-section` 工具
   - block_id: `root_xxx`
   - heading: `## API 参考`
   - file: `./updated-api.md`
...
1. 获取完整块树 - 调用 `lx-block-list-block-children` 工具
   - entry_id: `entry_xxx`

2. 更新目标块内容 - 调用 `lx-block-update-block` 工具
   - entry_id: `entry_xxx`
   - block_id: `blk_xxx`
   - update_text: `{"elements": [{"text": {"content": "新内容"}}]}`

3. 在指定位置插入新块（先转换 markdown） - 调用 `lx-block-convert-content-to-blocks` 工具
   - content: `## 新章节`
   - content_type: `markdown`
   - entry_id: `entry_xxx`
   - parent_block_id: `page_xxx`
   - descendant: `<转换结果>`

### 导入 Markdown 创建文档内容

1. 直接从文件导入（推荐） - 调用 `lx-block-import` 工具
   - block_id: `page_xxx`
   - file: `./doc.md`
   - chunk_size: `20`
